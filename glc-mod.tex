...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/3326729188-S1_1_1-standalone-001a.PNG}
...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/300662906-S1_1_2-networked-002a.PNG}
...
\emph{\textbf{Note}}: Press on the touchscreen only with your fingers.
...
buttons can be repalced with other menus or button selected from a
...
Constant Run\strut
\end{minipage} & \begin{minipage}[t]{0.56\columnwidth}\raggedright
Constant \#1, \#2 or \#3 can be selected for constant mode
operation.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.05\columnwidth}\raggedright
3\strut
...
Display Area\strut
\end{minipage} & \begin{minipage}[t]{0.56\columnwidth}\raggedright
Displays set point and process values of temperature and humidity.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
Displays available constant mode options; Constant \#1, \#2 or \#3 can
be selected for configuration.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.07\columnwidth}\raggedright
2\strut
\end{minipage} & \begin{minipage}[t]{0.25\columnwidth}\raggedright
...
\item
  ...
  bar over the keyboard indicates it is disabled. Press the keyboard (2)
  to enable it. Press Setup (3) to access the Setup submenu list.
...
\emph{\textbf{Note}} The system menu via the Hamburger button lists
...
\item
  ...
  access, these buttons are identical to the four menus in the menu bar.
\item
...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/3459346802-S1_6_2-web-login-option-002a.PNG}
...
Operation Mode\strut
\end{minipage} & \begin{minipage}[t]{0.67\columnwidth}\raggedright
This menu is locked out but is still accessible; when pressed/clicked,
the login dialog appears for the user to log in.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
4\strut
\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\raggedright
Monitor\strut
\end{minipage} & \begin{minipage}[t]{0.67\columnwidth}\raggedright
This menu display the current status of the GL controller.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright
5\strut
\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\raggedright
...
\item
  ...
  \emph{\textbf{Note}}: To cancel login, press X anytime during user
...
\includegraphics[width=0.95\textwidth]{figures/764530790-S3_2-step2.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  Press \textbf{Constant \#1} in the title bar to access
  \textbf{Constant \#1} configuration page. Three constant modes (No.~1,
  2, and 3) are available for configuration and operation. While
  Constant \#1 is selected for configuration, the following procedure
...
The following procedure outlines the step for setting up Constant \#1
with temperature and humidity. While Constant \#1 is selected for
...
\item
  Press \textbf{Constant \#1} in the title bar to access
  \textbf{Constant \#1} configuration page (see arrow). Three constant
  modes (No.~1, 2, and 3) are available for configuration and operation.
...
Default names for these time signals are Time Signal \#1, Time Signal
\#2, etc. They can be changed to reflect the description or purpose of
the external device(s). The following two figures depict examples of
time signal options set as default names and custom names.
...
\includegraphics[width=0.95\textwidth]{figures/1569113829-S3_5-step3-000b.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  To customize the name of Time Signal \#1, press the Full name field
  (1). Enter a new name using the pop-up input keyboard (2). \emph{Note:
  If the input keyboard does not pop up, it has not been enabled; go
  back to Step 2.} Press the Check button (3) to apply the setting.
  Press the Save button (4) to save the setting. Or, press the Back
  button (5) to cancel the setting (and press Yes to confirm the
  action).
...
\item
  ...
  (Constant \#1 setting is depicted in the figure below). In Program
  mode, time signal is identified by its number (discussed in Chapter
  5). As shown below, Time Signal \#1 (now called \emph{\textbf{Control
  Ext1}}) is in the OFF position. To turn it on, press the
  \textbf{RUNNING} button.
...
The default name of Constant \#1 is \textbf{constant\_1}. This name can
be changed to something descriptive.
...
\includegraphics[width=0.95\textwidth]{figures/136012415-S3_5-step3-002a.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  The same procedure applies to \textbf{Constant \#2} and
  \textbf{Constant \#3}.
\end{enumerate}
...
\item
  ...
  \textbf{Constant \#1} (see arrow). \textbf{\emph{Note:}} To check
  other options, it may require accessing the \textbf{Constant \#1}
  setup page.
...
\includegraphics[width=0.95\textwidth]{figures/1211506617-S3_8_1a-step1-000a.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  Press the \textbf{Constant \#1} panel and press Yes in the Attention
  box to confirm the action to start the operation.
\end{enumerate}
...
Constant mode, performing \textbf{Constant \#2} settings. Temperature
and humidity process values are also displayed.
...
3\strut
\end{minipage} & \begin{minipage}[t]{0.58\columnwidth}\raggedright
Displays time signals and their statuses. Note: On/Off manipulation can
be controlled in the \textbf{Set Mode} page.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\includegraphics[width=0.95\textwidth]{figures/2484552112-S3_9_1-page1-stdby-002a.PNG}

\textbf{Monitor Page 1}: Constant Mode

\includegraphics[width=0.95\textwidth]{figures/4067089529-S3_9_1-page1-const-1a.PNG}
...
Slots \#1, \#2 and \#4 are being occupied by programs with names and
number of steps displayed. Slots \#3 and \#5 are empty; hence, the plus
symbols. The GL controller system automatically opens a new slot for the
next program as soon as the current one is occupied by a program.
However, if programs were initially created in sequential order from
slot \#1 to \#4 (with slot \#5 automatically open for the next program),
and then a program in slot \#3 is deleted, that slot remains open. A new
program can be created to occupy slot \#3 again. The reason slot \#3
still remains open is because program \textbf{PAUL4} occupies slot \#4,
forcing slot \#3 to stay on the list, for it was pre-allocated before
slot \#4. This design was carefully considered to prevent randomly
reshuffling programs into different slots that might create confusion
for the operator, after deleting a program out of order. However, if
\textbf{PAUL4} is deleted, then slots \#4 and \#5 will collapse, forcing
only slot \#3 to remain open for the next new program.
...
open a file browser to import a file.

The Program Setup panel has three subpages to provide programming
features and options for the selected program, as depicted in the
following figure. Their features are listed and described in the table
that follows.

\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/640037717-S4_2_2-prgm-setup-page-002c.PNG}
\caption{Three subpages of the Program Setup panel}
\end{figure}\FloatBarrier
...
Program instructions can be created step by step\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.18\columnwidth}\raggedright
2\strut
\end{minipage} & \begin{minipage}[t]{0.25\columnwidth}\raggedright
...
\end{minipage} & \begin{minipage}[t]{0.48\columnwidth}\raggedright
Program instructions can be created by filling in the table
elements\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.18\columnwidth}\raggedright
3\strut
...
Details\strut
\end{minipage} & \begin{minipage}[t]{0.48\columnwidth}\raggedright
Details of each program operation mode can be configured\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

Details of these subpages are listed and described as follows.
...
(i.e., ramp, ramp rate or soak) can all be configured on the fly via
this method via the step panel that opens next to the selected step (see
arrow). Control modes and other parameters, such as refrigeration,
product temp deviation settings, control loop and time signals can be
configured via the \textbf{Details} button (wrench/gear icon).

\textbf{\emph{Note:}} The step panel is open and placed between the menu
bar and Step 1. The step panel (see arrow) in following figure is for
illustration only.
...
\textbf{Program Operation Details}: Program operation modes and other
parameters (such as program start condition, end mode, etc.) can be
configured in the \textbf{Details} subpage.
...
\includegraphics[width=0.95\textwidth]{figures/3823365595-S4_2_2-step3-000a.PNG}
  \caption{Open program editor}
  \end{figure}\FloatBarrier
\item
  Press the body of Step 1 (see arrow). Enter parameter values for
  temperature, humidity and operation time (hour, minute and second) in
  their designated fields in the step panel (displayed left to Step 1).
  Enable product temperature, ramp or soak control. Press the
  \textbf{Details} button (see arrow) to configure refrigeration
  capacity, time signals, deviation values for product temperature
  control, counter loop, etc. Press the checkmark button (see arrow) to
  apply the settings. To cancel the entire setting, press the X button.
...
\item
  ...
  \textbf{Move Before} or \textbf{Move After} button. Press the Step 2
  header (see arrow), then press the \textbf{Move Before} button to move
  Step 2 to before Step 1. As a result, the locations of Step 1 and Step
  2 are swapped. To move Step 2 to the position after Step 3, press the
  Step 2 header (see arrow), then press the \textbf{Move After} button.
  As a result, the locations of Step 2 and Step 3 are swapped.
...
\includegraphics[width=0.95\textwidth]{figures/1039181516-S4_3_2-step3-002a.PNG}
...
\item
  ...
  box; press on the box under the desired TS. Note: Time signal control
  can be turned On/Off after the parameter values are entered.
  Therefore, Step 1 instructions involve a 2-step process, with time
  signal control being the last step to complete.
...
After program construction is completed via the Step-Wise or Table-Wise
method, the \textbf{Details} page is next to complete. Start condition
and end mode are two important features in a program which must be
completed. Program name and other features, such as Abs. High/Low or
Dev. High/Low, may require configuration. Default program name in slot
\#1 is \textbf{Program \#1}; it can be edited (see arrow) to carry a
descriptive name. The \textbf{Save} button (see arrow) can be applied to
...
By default, program end mode is \textbf{Stop}, which means that when the
program completes executing its steps, it will stop the chamber. Options
such as hold the last step, turn on constant mode or execute another
program can be selected. The following two examples will illustrate how
to apply these features and operations.
...
\includegraphics[width=0.95\textwidth]{figures/3295364846-S4_3_4-step2-002a.PNG}
...
\includegraphics[width=0.95\textwidth]{figures/1996146587-S4_3_4-step5-002a.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  The default program name is \textbf{Program \#1}. Press the program
  name field (see arrow) and enter \textbf{Humidity Points} for program
  name. By default, program start mode consists of both temperature and
...
\includegraphics[width=0.95\textwidth]{figures/402760919-S4_3_5-step2-001a.PNG}
...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/1179308619-S4_4-step0-000a.PNG}
...
Export\strut
\end{minipage} & \begin{minipage}[t]{0.74\columnwidth}\raggedright
Export selected program(s) to an external USB device\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.20\columnwidth}\raggedright
Copy\strut
\end{minipage} & \begin{minipage}[t]{0.74\columnwidth}\raggedright
...
Programs can be exported to a USB device for backup. The program source
code is saved in JSON format. Program filename begins with chamber model
and program slot number; e.g., \textbf{GL-EPX-2-RM\_program\_1}.

The following procedure outlines the steps to export Program \#1
(created in Section 5.3.4) to a USB device.
...
\includegraphics[width=0.95\textwidth]{figures/4208992695-S4_4_1-image-step1a.PNG}
...
\includegraphics[width=0.95\textwidth]{figures/923119895-S4_4_1-step4-000a.PNG}
  \caption{Exporting Program \#1}
  \end{figure}\FloatBarrier
\item
  Program will be automatically stored in the external USB device.
...
\includegraphics[width=0.95\textwidth]{figures/2422246917-S4_4_2-step3a.PNG}
...
\includegraphics[width=0.95\textwidth]{figures/2806776619-S5_4_3-step1-002a.PNG}
  \caption{Selecting Program \#3 for Move/Swap operation}
  \end{figure}\FloatBarrier
\item
  The blue pop-up notification (in lower right) indicates Program \#3
  has been selected for Move/Swap operation. To cancel the action, press
...
\includegraphics[width=0.95\textwidth]{figures/4087364197-S5_4_3-step4-001.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  Program \#3 has been move into Program \#4. Slot \#3 is now empty, as
  shown in the figure.
...
\includegraphics[width=0.95\textwidth]{figures/534048827-S5_4_3-step5-001.PNG}
  \caption{Program \#3 has been move and slot\#3 is now empty}
  \end{figure}\FloatBarrier
\end{enumerate}
...
\includegraphics[width=0.95\textwidth]{figures/147643888-S5_4_4-step1-001a.PNG}
...
\includegraphics[width=0.95\textwidth]{figures/3019997897-S5_4_5-step1-001a.PNG}
...
\includegraphics[width=0.95\textwidth]{figures/1936494498-S5_4_6-step1-001a.PNG}
...
\item
  ...
  with empty slot \#5, leaving just slot \#3 ready to be populated by a
  new program.
...
For demonstration, Program \#2 (``TempTest'') will be selected to remove
a single step, followed by removing a range of steps. Program \#2
consists of 13 steps. Step 2 will be removed as a single step; step 5
through step 7 will be removed as a range of steps.

\textbf{\emph{Note:}} To prepare Program \#2 to contain 13 steps to
follow along with this demonstration, refer to Section 5.3.4. It is not
...
\includegraphics[width=0.95\textwidth]{figures/3233949427-S5_5_1-step3-001a.PNG}
...
\item
  ...
  the chosen step.
\item
  Press Yes in the dialog box to confirm the action.
\end{enumerate}

The following procedure outlines the steps to cut Step 3-5 and paste
them in front of Step 1.
...
The following procedure outlines the steps to pin Program \#1 to the
home page. Step 1 of the program will be the default step to start the
execution.
...
\includegraphics[width=0.95\textwidth]{figures/3269330728-S8_3_7_step2-002a.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  Select one Constant mode (say, Constant \#1); make notes of its
  original settings. Set target temperature to a minimum 70 degrees and
  set humidity and refrigeration control OFF. (It is unnecessary with
...
\item
  Start Constant \#1 again for 15 minutes with the door slightly
  cracked.
\item
...
Set the main power breaker in the OFF position.
...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/3953050056-S6_1_4-LAN-001a.PNG}
...
\emph{\textbf{Note}}: Refer to Section 2.6.2 for details on the
localhost nad how to log in or out of the account.
...
\textbf{\emph{Procedure:}} 1. Set the correction value of the offset
adjustment. 2. Set the correction value to be added to or subtracted
from the sensor in the range of +/- 5 °C. 3. After entering the value,
the value corrected to the sensor input value is displayed at the
Process Value.

\textbf{\emph{Procedure:}} 1. Press Set/Calibrate sensor on the
...
\item
  ...
  ``DAP'' indicated by callout 2, ``Start'' by callout \#3 and ``Stop''
  by callout \#4.
...
\includegraphics[width=0.95\textwidth]{figures/3022334899-S6_7-step2-002a.PNG}
  ...
  \end{figure}\FloatBarrier
\item
  Save the setting using the \textbf{Save} icon (callout \#5) and
...
Currently, English is the only language supported by the GL controller.

\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/1827366379-S6_11-language-support-001.PNG}
\caption{Language support is English only}
\end{figure}\FloatBarrier
...
\textbf{NOTE}: Many operations associated with the macros require that
the GL controller has access to the Internet.
...
any alarm occurrences. Note: This sample macro script is presented to
illustrate the flexibility of the macro editor. There are numerous ways
to achieve the same the task noted here.
...
\item
  ...
  \item
    ...
    NOTE: \textbf{PROG2TEST} must be made available on the program list.
  \item
...
\includegraphics[width=0.95\textwidth]{figures/1688259577-macro-example-002a.PNG}
  ...
  \item
    ...
    NOTE: \textbf{PROG3TEST} must be created and is available on the
    program list.
  \item
    The last option on step number is set as default for the trigger to
    apply at any step.
...
\includegraphics[width=0.95\textwidth]{figures/178688635-S7_3_1-step2-001a.PNG}
...
\subsection{8.4. Static Network using Fallback
IP}
...
\includegraphics[width=0.95\textwidth]{figures/691940129-S7_5-gl-static-network-002a.PNG}
...
\begin{figure}[hptb!]
\centering
\includegraphics[width=0.95\textwidth]{figures/3828024261-S7_4_2-static-ip-network-0011.PNG}
\caption{GL controller static network with multiple PCs}
\end{figure}\FloatBarrier

\textbf{\emph{Procedure:}} 1. Repeat step 1 in the procedure of Section
8.3 (above) and set up Ethernet connection as depicted in the above
diagram. 2. Follow steps 3-10 in Section 8.4.1 to set up IP address for
each PC on the network. 3. To access the GL controller on each PC, open
a Web browser on the PC and navigate to: http://192.168.0.83/
...
\includegraphics[width=0.95\textwidth]{figures/754414513-S7_5-gl-static-network-001a.PNG}
  \caption{Static network configuration}
  \end{figure}\FloatBarrier
\item
...
network. Note: All EWC device hostnames and IP addresses used in this
example are for illustration purposes only. You must use the hostnames
and IP address of your EWC devices found on your network.
...
\item
  Starting from the Network Monitor page, click the \textbf{ALL
  CHAMBERS} tab. Note: It is not necessary to select \textbf{ALL
  CHAMBERS}, but selecting it allows a complete manual configuration (as
  will be apparent at step 4).\\
\item
  Click the \textbf{Edit Layout} (pen icon).
\item
...
\includegraphics[width=0.95\textwidth]{figures/1146931544-ex4-step4.PNG}
  ...
  \item
    ...
    from the list.
  \item
    Enter \textbf{MyWebDev25} in the Display Name field. Note: To
    display the EWC device by its hostname, \textbf{WebDev25} may be
    entered here. This field cannot be left blank or an \textbf{INVALID
