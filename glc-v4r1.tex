...
% using explicit options in \includegraphics[width=0.95\textwidth][width, height, ...]{}
...
the GL controller; it is primarily intended for trained engineers and
technicians. Please carefully read this manual to ensure satisfactory
and safe usage of the GL controller software and test chamber.

\hypertarget{for-restricted-use}{%
...
\emph{Definition of an experienced engineer}: A person who understands
the purpose of chamber operations and controls; who has received
training in the operation method, daily maintenance and inspections,
etc.; who can foresee and prevent risks associated with common sources
of hazards, such as electricity, etc.

\hypertarget{safety-indications}{%
\section{Safety Indications}

The following safety indications are used throughout this manual. The
following table lists different signs that represent the degrees of
danger to users.
...
The following labels indicate information on physical damage and
...
of damage to equipment and facilities or environmental pollution, if the
equipment is handled incorrectly.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
and causing injury to the operator. 2NEWLINE2 Ensure the equipment
clearly displays that it is being operated remotely; and also properly
notify the operator.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\hypertarget{display-used-in-this-manual}{%
...
\hypertarget{gl-controller-with-temperature-and-humidity}{%
...
\hypertarget{gl-controller-operation-options}{%
...
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.48\columnwidth}\raggedright
The HMI provides complete control and operation of the GL controller and
the chamber.\strut
\end{minipage} & \begin{minipage}[t]{0.46\columnwidth}\raggedright
...
\end{minipage} & \begin{minipage}[t]{0.46\columnwidth}\raggedright
Update on the GL controller is provided via Mender IO cloud service, if
the GL system has access to the Internet.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\hypertarget{gl-controller-standalone-system}{%
...
dedicated HMI on the chamber, as depicted in the following diagram.
...
\hypertarget{gl-controller-networked-system}{%
...
Refer to Section 2.10 on how to enable the GL controller system to allow
remote access via a Web Browser on the local network.
...
\hypertarget{organization-of-gl-controller-user-interface}{%
...
Set Buttons\strut
\end{minipage} & \begin{minipage}[t]{0.65\columnwidth}\raggedright
Displays quick setting buttons for user's login protection, date/time
adjustment and language selection\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.04\columnwidth}\raggedright
6\strut
\end{minipage} & \begin{minipage}[t]{0.23\columnwidth}\raggedright
...
icon) allows the user to edit the layout; e.g., the four menu buttons
can be replaced with other menus or button selected from a different
menu for quick access and operation.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.04\columnwidth}\raggedright
7\strut
\end{minipage} & \begin{minipage}[t]{0.23\columnwidth}\raggedright
...
\includegraphics[width=0.95\textwidth]{figures/1269155203-S4_3_5-setup-menu-001a.png}
...
\end{minipage} & \begin{minipage}[t]{0.65\columnwidth}\raggedright
Provides On/Off options for users to set HMI automatic login
protection\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.04\columnwidth}\raggedright
4\strut
...
submenus altogether is via the Hamburger menu (or button) in the status
bar, as depicted below.
...
\hypertarget{operating-panel-elements-and-controls}{%
\section{Operating Panel: Elements and
Controls}

\hypertarget{menu-and-common-operation-buttons}{%
...
\hypertarget{turning-the-power-onoff}{%
...
\hypertarget{turning-on-the-power}{%
...
\hypertarget{turning-off-the-power}{%
...
\hypertarget{user-accounts-system-menu-and-login-status}{%
...
\hypertarget{login-status-user-accounts}{%
...
diagonal bar over the user icon to indicate the user has logged out.
...
\hypertarget{how-to-log-in-via-hmi}{%
...
\hypertarget{how-to-log-in-via-remote-access}{%
...
\hypertarget{three-different-ways-to-access-gl-controller-system-menu}{%
...
\hypertarget{system-menu-via-menu-bar}{%
...
\hypertarget{system-menu-via-menu-button}{%
\subsection{System Menu via Menu
Button}
...
\hypertarget{system-menu-via-hamburger-button}{%
...
\hypertarget{setting-alarm-and-warning-buzzer}{%
\section{Setting Alarm and Warning
Buzzer}
...
\endhead
\begin{minipage}[t]{0.97\columnwidth}\raggedright
\textbf{To prevent delayed warning detection, leave the audible alert on
at all times.} 2NEWLINE2 If the beep is turned off, alarm and warning
are not notified with sound and are indicated only with red blinking of
the operation lamp and on warning occurrence screen. 2NEWLINE2
\textbf{Set the beep volume in accordance with the ambient
environment.}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\hypertarget{gl-controller-and-chamber-datetime}{%
...
\hypertarget{remote-access-and-set-protection}{%
\section{Remote Access and Set
Protection}

The \textbf{Set Protection} page allows the operator to enable/disable
the HMI autologin to protect the system against unauthorized users. It
also offers two remote-operation options when the GL controller system
is part of the local network, namely, (1) remote access via a Web
browser on a PC or other web-based device on that local network, and (2)
remote access via direct communication with GL native text commands
through TCP/IP (Transmission Control Protocol that uses Internet
Protocol) or serial interface.

These two options are configurable on the HMI display as depicted in the
following figure.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/2021071933-S6_5-set-protection-001a.png}
\caption{Set Protection page with options to enable or disable remote
access}
\end{figure}\FloatBarrier
...
HMI autologin; User has option to enable or disable HMI auto login as an
extra protection.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.07\columnwidth}\raggedright
2\strut
\end{minipage} & \begin{minipage}[t]{0.88\columnwidth}\raggedright
Remote communication via TCP/IP or serial interface can be controlled
with Disabled, Monitor Only, and Monitor and Control options. When
disabled, remote communication is blocked; when set to Monitor Only,
monitor commands can be issued; when set to Monitor and Control, both
monitor and set commands can be issued.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.07\columnwidth}\raggedright
3\strut
\end{minipage} & \begin{minipage}[t]{0.88\columnwidth}\raggedright
Remote access to the GL controller system with full control and
operation via a Web-based device; e.g., access via a Web browser running
on PC or handheld device on the local network.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.07\columnwidth}\raggedright
4\strut
\end{minipage} & \begin{minipage}[t]{0.88\columnwidth}\raggedright
Configuration options for Multicast DNS (mDNS) name resolution on the
local network\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\textbf{\emph{Note:}} Configuration options No.~1, 3 and 4 must be
performed directly on the HMI. The following figure depicts the
\textbf{Set Protection} page when accessed via a Web browser.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/3007255201-S6_5-set-protection-web-002.png}
\caption{GL controller UI on a Web browser}
\end{figure}\FloatBarrier

The following sections provide a quick way to enable (or disable) the
remote communication (via TCP/IP or serial) and remote access via a Web
browser running on a PC or a handheld device on the local network. For
complete operation of the \textbf{Set Protection} page, refer to Section
6.5.

\hypertarget{remote-communications-option}{%
\subsection{Remote Communications
Option}

TCP/IP is the default communication protocol used by the GL controller.
It offers both speed and convenience over its serial counterpart. The
following procedure provides steps to enable TCP/IP communication for
Monitor and Control. If remote serial communication is required, this
procedure also applies with appropriate button selected. Refer to
Section 7.1.6 for details on remote communication via an SSH terminal
...
\def\labelenumi{\arabic{enumi}.}
\item
  Log in as user with administrative privilege; the localhost user that
  the system uses is a viable option.
\item
  Access the Home page and press \textbf{Set Protection (User Settings)}
  (see arrow).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/3119455434-SN2_10_1-set-protection-001.png}
  \caption{Accessing \textbf{Set Protection} settings option}
  \end{figure}\FloatBarrier

  \textbf{\emph{Note:}} If this button is not available, select
  \textbf{Setup} (in the menu bar), then press the \textbf{Set
  Protection (User Settings)} button (see arrow).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/1571179245-SN2_10_1-step3-001a.png}
  \caption{Accessing \textbf{Set Protection} page from the Setup menu}
  \end{figure}\FloatBarrier
\item
  Press the \textbf{Monitor and Control} button in the Remote TCP/IP
  Communication field (see arrow)

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/936457416-SN2_10_1-step4-001a.png}
  \caption{Enabling remote TCP/IP communication}
  \end{figure}\FloatBarrier
\end{enumerate}

Refer to Section 7.1.6 on how to use remote TCP/IP communication via an
SSH terminal emulator, called PuTTY.

\textbf{\emph{Note:}} Remote TCP/IP communication protocol operation is
unaffected by the setting of network remote access for a Web browser
(discussed next). Therefore, remote TCP/IP communication can be enabled
and used independently of the Web-browser remote access setting.

\hypertarget{network-remote-access}{%
\subsection{Network Remote
Access}

The following procedure provides steps to enable local network remote
access such that the GL controller to be accessed and controlled
remotely via a Web browser running on a PC or a web-based device on the
local network.
...
\item
  ...
  the system uses is a viable option.
\item
  Access the Home page and press the \textbf{Set Protection (User
  Settings)} (see arrow).
...
\includegraphics[width=0.95\textwidth]{figures/1571179245-SN2_10_1-step3-001a.png}
  ...
  \end{figure}\FloatBarrier
\item
  Press the \textbf{HTTP/HTTPS Access} button (see arrow), followed by
  \textbf{Yes} to confirm the action.

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/3200028559-SN2_10_2-step3-001a.png}
  \caption{Enabling local network remote access on GL controller}
  \end{figure}\FloatBarrier
\item
  The GL controller system will reboot to apply the setting.
\item
  To confirm the setting following the reboot, repeat Step 2.
\item
  To find out what IP address or hostname the GL controller system is
  using, refer to Section 7.1.4 for details.
\item
  To access the GL controller system from a PC on the local network,
  launch a Web browser (e.g., Chromium, MS Edge, Firefox, Google
  Chrome), then in the URL field, enter:
  ...
  where \emph{IP-address} or \emph{hostname} was obtained in Step 6
  (above).
\end{enumerate}

Here are two examples of GL controller UI on Google Chrome accessed via
its IP address and its hostname, respectively.
...
\includegraphics[width=0.95\textwidth]{figures/2508351595-SN2_10_2-web-access-example1a.png}
\caption{GL controller on a Web browser}
\end{figure}\FloatBarrier
...
\includegraphics[width=0.95\textwidth]{figures/1183325582-SN2_10_2-web-access-example2a.png}
\caption{GL controller on a Web browser}
\end{figure}\FloatBarrier

\hypertarget{account-recovery-e-mail}{%
\section{Account Recovery
E-mail}

The administrator's account (username \emph{admin}) and password play an
important role in managing the GL system security and operation. With
multiple accounts and users on the system, the admin account must be
kept secure so that no unauthorized users have access to it. If the
account is compromised or its password is forgotten, a password recovery
option can help reset the admin account.

\hypertarget{set-account-recovery-e-mail}{%
\subsection{Set Account Recovery
E-mail}
...
\textbf{\emph{Note:}} The following procedure outlines steps starting
from a cold start of the GL chamber. If the GL system is already turned
...
\item
  Turn on the circuit breaker of the instrumentation by setting the main
  power breaker in the ON position (see Section 2.5.1).
\item
  Log out of localhost by pressing the user localhost icon (see arrow)
  followed by the \textbf{Log Out} button (see arrow).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/4222829398-S2_11_1-step2-logout-localhost-001a.png}
  \caption{Logging out of the localhost account}
  \end{figure}\FloatBarrier
\item
  Enable the floating keyboard; see Section 2.4.4.
\item
  Log in to the admin account using admin credentials, followed by the
  \textbf{Submit} button (see arrow). Refer to Section 2.6.3 for
  details.

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/579487506-S2_11_1-step4-logging-into_admin-001a.png}
  \caption{Logging in to admin account}
  \end{figure}\FloatBarrier
\item
  Access the hamburger button in the status bar (see callout 1), select
  Configuration (see callout 2), followed by Email (see callout 3).
  \emph{\textbf{Note:} Another way to access Email page is from the home
  page \textbf{Setup} button or the \textbf{Setup} menu, then select
  Configuration and Email.} Our approach here is more direct.

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/1911056020-S2_11_1-step5-system-menu-001ab.png}
  \caption{Accessing the Email setup page}
  \end{figure}\FloatBarrier
\item
  To set up admin account recovery, enter your e-mail address in the
  \textbf{Account Recovery E-Mail} field (see top-right arrow). To set
  alarm alert via e-mail, enter the recipient e-mail address in the
  \textbf{Alarm Recipients} field (see bottom-right arrow). Additional
  recipient e-mails may be added using the \textbf{Add} button.

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/3932164582-S2_11_1-step6-email-setting-001a.png}
  \caption{Entering e-mail address for admin account recovery}
  \end{figure}\FloatBarrier
\item
  If the GL system is connected the your main network which has access
  to the Internet, an e-mail test can be done by applying the e-mail
  button in the bottom of the menu bar (see arrow). If e-mail settings
  are all correct, apply the \textbf{Save} button to save the setting.
  \textbf{\emph{Note:}} \emph{To cancel the Email setup procedure, apply
  the back-arrow button in the menu bar.}
\item
  Log out of the admin account.
\end{enumerate}

\hypertarget{how-to-recover-admin-account-and-password}{%
\subsection{How to Recover Admin Account and
Password}

This section outlines the steps to recover the admin account and
password. It should be noted that the actual process of account recovery
is to reset the admin account to factory setting; and from there you can
change the admin password to something more secure. For this reason,
Section 2.11.1 should be done first and foremost to limit security
breach. Once an e-mail address has been set and stored in the system,
the recovery procedure will be sent to that e-mail address.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Follow the procedure in Section 2.11.1, Step 1 through Step 3.\\
\item
  Enter the admin username (admin), then press the \textbf{Admin Account
  Recovery} button (see arrow).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/1890319025-S2_11_2-step2-001a.png}
  \caption{Accessing the account recovery page}
  \end{figure}\FloatBarrier
\item
  \begin{enumerate}
  ...
  \tightlist
  \item
    If you have set up an account recovery in Section 2.11.1 with your
    e-mail address, and your GL system is connected to the main network,
    select the \textbf{Send Recovery E-Mail} button (see callout 1). A
    link to reset the admin account is sent to your e-mail account; open
    the e-mail and click on the link to access the GL system and set the
    admin password. (b) If you did not set up an account recovery in
    Section 2.11.1, copy the contents in the \textbf{Message} box (see
    callout 2) and compose an e-mail based on the given instruction for
    \textbf{To} and \textbf{Subject}, ESPEC will respond with a link to
    access your GL system. (c) Or, with your smartphone, you can use the
    QR code (see callout 3) to send a message to ESPEC for account
    recovery.
  \end{enumerate}
...
\includegraphics[width=0.95\textwidth]{figures/2696323673-S2_11_2-step3-reset-admin-account-002a.png}
  \caption{Resetting the admin account based on the three options
  available to you}
  \end{figure}\FloatBarrier
\end{enumerate}

\hypertarget{language-selection}{%
\section{Language Selection}

The GL controller supports multiple languages. The following list
outlines the different languages currently available in the GL
controller user interface: \emph{English}, \emph{Spanish},
\emph{Italian}, \emph{Korean}, \emph{Portuguese}, \emph{Simplified
Chinese}, \emph{Traditional Chinese}, \emph{German}, \emph{French},
\emph{Japanese}, \emph{Polish}, \emph{Vietnamese}.

The default language is English. The list of available and supported
languages on the GL controller system is under the \textbf{Set Language}
submenu, as depicted in the following figure: \textbf{Setup}
-\textgreater{} \textbf{Configuration} -\textgreater{} \textbf{Set
Language}.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/380079532-S2_12-language-selection-001.png}
\caption{A list of available and supported language on GL controller
system}
\end{figure}\FloatBarrier

This section outlines the procedure to set and select a desired language
for the GL controller user interface. Language selection can be set
under any user account.

\textbf{\emph{Procedure 1:}} Select a language from within the Home
page.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Press the \textbf{Set Language} button in the home page (see arrow).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/602983469-S2_12-step1-Set-Language-button-001a.png}
  \caption{Accessing the Set Language page}
  \end{figure}\FloatBarrier
\item
  Select the desired language from the list (in the \textbf{Set
  Language} page).
\end{enumerate}

\textbf{\emph{Procedure 2:}} Select a language via the menu bar.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Press \textbf{Setup} in the menu bar.
\item
  Press \textbf{Configuration}
\item
  Press \textbf{Set Language}
\item
  Select the desired language from the list.
\end{enumerate}

\textbf{\emph{Procedure 3:}} Select a language via the hamburger button.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Press the hamburger button (see callout 1); select
  \textbf{Configuration} (callout 2), followed by \textbf{Set Language}
  (callout 3)

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/2719949474-S2_12-procedure3a.png}
  \caption{Accessing Set Language under the system menu}
  \end{figure}\FloatBarrier
\item
  Select the desired language from the list (in the \textbf{Set
  Language} page).
...
setting before conducting a test.} 2NEWLINE2 There is a possibility that
the temperature cannot be maintained and the specimen will be damaged.
2NEWLINE2 \textbf{If the refrigeration capacity setting is changed
during operation, control may become unstable, and the temperature and
humidity fluctuation of the specification performance may not be
guaranteed. (The refrigeration capacity setting cannot be changed during
program operation.)}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\hypertarget{automatic-control-of-refrigeration-capacity}{%
...
both settings are fixed and cannot be changed. 2NEWLINE2 Even if the
...
\hypertarget{constant-value-operation}{%
...
\hypertarget{constant-value-operation-definition}{%
\section{Constant Value Operation:
Definition}
...
\hypertarget{constant-temperature-humidity-setting}{%
...
\hypertarget{refrigeration-setting}{%
...
\endhead
\begin{minipage}[t]{0.97\columnwidth}\raggedright
For details on refrigeration setup, see Section 3.1. 2NEWLINE2 When you
want to change the mode from Auto to Manual during operation, manually
set the capacity of the refrigerator using one of these options: 25\%,
50\%, 75\% or 100\%. When refrigeration is changed from Auto to Manual
(with specified capacity), refrigeration stops immediately. This may
cause control to become unstable. 2NEWLINE2 When refrigeration is set to
Auto, the manual setting is not reflected in the operation of the
refrigeration system.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\hypertarget{highlow-limit-setting}{%
...
\hypertarget{constant-name}{%
\section{Constant Name}
...
\hypertarget{product-temperature-control-options}{%
...
\hypertarget{startstop-operation}{%
...
\endhead
\begin{minipage}[t]{0.97\columnwidth}\raggedright
When testing electronic components, ensure condensation has not affected
the specimen. 2NEWLINE2 Condensation occurs when the surface temperature
of the specimen is lower than the dew-point temperature of the air in
the test area. To avoid condensation of the specimen, it is necessary to
perform temperature-only operation in advance and start
temperature/humidity control operation after the surface temperature of
the specimen reaches equilibrium with the temperature in the test area.
2NEWLINE2 In addition, condensation can be avoided by using
humidification delay control. If the temperature and humidity in the
test area is 85 °C and 85\% RH, respectively, the dew-point temperature
of the air in the test area is 80.9 °C. Therefore, condensation occurs
if the surface temperature of the specimen is less than 80.9 °C.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\hypertarget{starting-constant-value-operation}{%
...
\hypertarget{stopping-constant-value-operation}{%
...
installation location. 2NEWLINE2 To avoid condensation effect, return
the temperature in the test area to ambient temperature before stopping
operation.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\hypertarget{monitoring-constant-value-operation}{%
...
\hypertarget{home-page-standard-display-temperaturehumidity}{%
...
\caption{Monitor Page 1: Information page of Standby mode}
\end{figure}\FloatBarrier
...
\hypertarget{monitor-detailed-parameters}{%
...
\hypertarget{monitor-preview-and-program-output}{%
...
\hypertarget{monitor-trend-graph}{%
\subsection{Monitor: Trend Graph}
...
\hypertarget{trend-graph-manipulation-buttons}{%
...
\hypertarget{how-to-handle-downloaded-data-file}{%
...
\hypertarget{program-setup-panel}{%
...
\hypertarget{creating-a-new-program}{%
...
\hypertarget{step-wise-programming}{%
...
air temperature limit. 2NEWLINE2 When the P.Temp process value is
further from the P.Temp set point, the controller will ``drive'' the air
temperature set point to this maximum upper (or lower) deviation limit.
As the P.Temp process value gets closer to the P.Temp set point, the air
temperature set point will change such that the effect is less than the
maximum upper (or lower) deviation limit. 2NEWLINE2 The High and Low
deviation can be adjusted via the \textbf{Details} tab by setting the
Dev.High and Dev.Low values in the High \& Low Limits panel in the
appropriate fields. Refer to Section 5.3.3.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
process values, such as a recorder or a temperature meter. 2NEWLINE2 (1)
Run the chamber with the selected test product to the desired P.Temp set
point with product temperature control turned OFF. Run this ``test'' in
constant mode. 2NEWLINE2 (2) When the air temperature has reached the
desired set point, record the temperature difference between the product
and the air temperature. This temperature difference will be used as the
deviation limit setting. 2NEWLINE2 (3) Enter the recorded value (from
Step 2) into the (+) setting for product heat up. Enter the value into
the (-) setting for product cool down. 2NEWLINE2 (4) Execute this
procedure for heat up and cooling cycles in order to determine the upper
(+) and lower (-) deviation limits. 2NEWLINE2 (5) During product
temperature control, the response characteristic of control differs
depending on the heat capacity, heat-transfer coefficient, and thermal
conductivity of the control target specimen. Since the required control
parameters are also changed, set the appropriate values
accordingly.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
ramping. 2NEWLINE2 Additionally, there are some specific circumstances
where you may be required to use manual refrigeration control to run a
specific test profile. An example of this kind would be to use manual
refrigeration to execute a ``pre-chill'' function with the refrigeration
system of your test chamber. 2NEWLINE2 This ``pre-chill'' function can
be used to maximize the temperature change rate of the chamber. This
setting should be used when transitioning from a set point above ambient
to a low temperature. The pre-chill setting is accomplished by setting
the manual refrigeration control to 25\% output 3-5 minutes before the
transition taking place. This will start the refrigeration system and
pre-cool the cascade of the system to ensure that maximum capacity is
available for the transition. When the temperature transition is
started, the refrigeration should be returned to ``auto'' control. This
method can be used in constant mode or program mode operation. 2NEWLINE2
\textbf{\emph{Note:}} When refrigeration is set Auto, the manual setting
is not reflected in the operation of the refrigeration system. You
cannot change the refrigeration capacity during operation. Set the
refrigeration capacity when setting the program prior to
operation.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
after), (2) Repeating end step, (3) Repeat cycle 2NEWLINE2 When counter
setup is enabled, steps from the start step (1) through the end step (2)
are repeated until the repeat cycle count (3) is completed. To set up
...
selecting start step and end step are: 2NEWLINE2 (A) 1 LTH2 start step
\textless{} number of registered steps 2NEWLINE2 (B) Repeating start
step \textless{} repeating end step; end step \textgreater{} 1 2NEWLINE2
(C) There is no limit set for the number of repeat cycle. 2NEWLINE2
\textbf{\emph{Note:}} The total number of operations resulting from the
repeating start step (1) to the repeating end step (2) is the number of
repeat cycle (3) plus 1. This means the first loop must be completed
before the repeat cycle commences. Therefore, if step 1 and step 3 are
to repeat five times, the input number of cycle is four (i.e., 4 + 1).
Non-continuous steps can be marked to be executed repeatedly. Multiple
loops (or counters) can exist within a program. 2NEWLINE2 In the
Step-Wise Program editor, each counter loop is marked with a bracket
that covers the start step and end step. To mark the start step, select
the desired step and invoke the \textbf{Details} subpage to configure
the counter. 2NEWLINE2 Refer to Section 5.3.2 for details on counter
setup in Table-Wise Programming.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\hypertarget{table-wise-programming}{%
...
\hypertarget{example-1-creating-program-using-step-wise-method}{%
\subsection{Example 1: Creating Program using Step-Wise
Method}
...
\includegraphics[width=0.95\textwidth]{figures/1698428879-S6_2_3-figure1.PNG}
  ...
  \textbf{Upper deviation temperature limit}: This setting is designed
  to protect specimens against damage by heat. It also triggers a safety
...
\hypertarget{example-2-creating-program-using-table-wise-method}{%
\subsection{Example 2: Creating Program using Table-Wise
Method}
...
\hypertarget{example-3-setting-multiple-counters-in-a-program}{%
...
\hypertarget{managing-a-program-export-import-and-all-that}{%
...
\hypertarget{exporting-a-program}{%
...
\hypertarget{importing-a-program}{%
...
\hypertarget{movingswapping-a-program}{%
\subsection{Moving/Swapping a
Program}
...
\hypertarget{renaming-a-program}{%
...
\hypertarget{previewing-a-program}{%
...
\hypertarget{deleting-a-program}{%
...
\hypertarget{deleting-steps-in-a-program}{%
\subsection{Deleting Steps in a
Program}
...
\hypertarget{moving-steps-in-a-program}{%
\subsection{Moving Steps in a
Program}
...
\hypertarget{inserting-steps-in-a-program}{%
\subsection{Inserting Steps in a
Program}
...
\hypertarget{cuttingpasting-steps-in-a-program}{%
\subsection{Cutting/Pasting Steps in a
Program}
...
\hypertarget{executing-a-program}{%
...
\hypertarget{executing-a-program-in-the-program-run-panel}{%
...
\hypertarget{executing-a-program-from-within-the-home-page}{%
...
\hypertarget{how-to-pin-a-program-to-the-home-page}{%
...
\hypertarget{manipulating-a-program-during-its-execution}{%
...
\hypertarget{monitoring-program}{%
...
\hypertarget{monitor-detailed-parameters-1}{%
\subsection{Monitor: Detailed
Parameters}
...
\hypertarget{monitor-preview-and-program-output-1}{%
\subsection{Monitor: Preview and Program
Output}
...
\hypertarget{monitor-trend-graph-1}{%
\subsection{Monitor: Trend Graph}
...
\hypertarget{trend-graph-manipulation-buttons-1}{%
\subsection{Trend Graph Manipulation
Buttons}
...
\hypertarget{program-mode-one-time-timer}{%
...
\hypertarget{removing-or-adding-a-timer}{%
...
\hypertarget{constant-mode-one-time-timer}{%
...
\hypertarget{program-mode-weekly-repetition}{%
...
\hypertarget{program-mode-daily-repetition}{%
...
\hypertarget{multiple-timers-one-time-or-repeat-dailyweekly}{%
\subsection{Multiple Timers: One-Time or Repeat
Daily/Weekly}
...
bear a date/time stamp of the GL controller as shown in the status bar
on the HMI display (see Section 2.4.2). For practical purposes,
date/time of the GL controller should reflect the current date and local
time zone. Refer to Section 7.2 for details on how to adjust date/time
on the GL controller.

The \textbf{Set Sampling} page consists of three main components: (1)
data logging frequency, (2) data logging mode and (3) logging items. It
is recommended to clear (i.e., delete) the data log regularly by
applying the \textbf{Delete all logged Data} button, designated by arrow
No.~4 in the following figure, to avoid mass data accumulation both to
...
Data logging is controlled by the sampling frequency and chamber
operating mode designated by Item 1 and Item 2 in the table,
respectively. The default sampling frequency occurs at a 5-second
interval. An arbitrary value can be used to set the sampling frequency
in Item 1. Two operating modes are available to control data logging:
\textbf{Always} and \textbf{Running}. When \textbf{Always} is selected,
data will be logged at all time. When \textbf{Running} is selected, data
will be logged only when the chamber is running, both in Constant and
Program mode.
...
based on the check mark placed in their respective boxes (see figure
below). This page is scrollable to allow all available items be selected
and included in the data log. Color decoration can be selected for plot
legends. Different logs for a trend graph can be created using the (+)
button. The y-axis (vertical) scaling can be adjusted via the Y Axis Max
and Y Axis Min buttons. Detailed steps for these settings are discussed
...
\hypertarget{set-data-log-in-any-mode}{%
...
\hypertarget{set-data-log-in-program-mode}{%
...
\hypertarget{example-creating-custom-log}{%
...
\hypertarget{alarmoperation-history}{%
...
\hypertarget{alarms-and-warnings}{%
\subsection{Alarms and Warnings}
...
\hypertarget{list-of-operating-modes}{%
...
\hypertarget{operation-statistics-donut-chart}{%
...
\hypertarget{set-protection-user-settings}{%
...
This page also offers other features to extend or limit control on the
GL controller system for remote access. The GL controller system offers
two unique remote-operation options when it is connected to the local
network: (1) remote access via a Web browser on a PC or other web-based
device on that local network, and (2) remote access via direct
communication with GL native text commands through TCP/IP (LAN
communiation) or serial interface. These two options are configurable on
the HMI.
...
2\strut
\end{minipage} & \begin{minipage}[t]{0.88\columnwidth}\raggedright
Three control options are available for remote communication via TCP/IP
or serial interface: (i) Disabled, (ii) Monitor Only, and (iii) Monitor
and Control. When option (i) is selected (Disabled), remote
communication is blocked; when option (ii) is selected (Monitor Only),
only monitor commands can be issued; when option (iii) is selected
(Monitor and Control), both monitor and set commands can be
issued.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.07\columnwidth}\raggedright
3\strut
...
\end{minipage} & \begin{minipage}[t]{0.88\columnwidth}\raggedright
Configuration options for Multicast DNS (mDNS) name resolution on the
local network. mDNS provides a quick way to resolution hostname on the
local network; access to a device via its hostname need only include the
dot local (.local) attached to the target hostname. This option is
secondary to No.~3.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

\textbf{\emph{Note:}} Configuration options No.~1, 3 and 4 (in the above
table) can only be performed directly on the HMI. If the GL controller
system is accessed via a Web browser, only remote TCP/IP or serial
communication option is available, as depicted in the following figure.
...
The following procedures provide a quick way to enable (or disable) HMI
autologin, remote TCP/IP or serial communication and remote access via a
Web browser.

\hypertarget{hmi-autologin}{%
\subsection{HMI autologin}

The HMI automatic login feature can be disabled or enabled for extra
protection.
...
\tightlist
\item
  With the system already turned On, access the Home page.
\item
  Press the \textbf{Set Protection (User Settings)} button.
\item
  Press the Off button
\end{enumerate}

\hypertarget{remote-tcpip-or-serial-communication-options}{%
\subsection{Remote TCP/IP or Serial Communication
Options}

Direct communication with the GL controller at the text-command level is
achievable with the remote TCP/IP or serial communication protocol.
Remote communication option can be set to issue only monitor commands or
monitor and control commands; it can be disabled completely to lock out
communication. By default, the GL controller uses TCP/IP for remote
communication to offer both speed and convenience. If remote serial
communication is desired and it is available as an optional feature on
your GL controller system, it can be set via the appropriate button
(refer to the previous figure in Section 6.5).
...
Refer to Section 7.1.6 for details on how to use remote TCP/IP
communication via an SSH terminal emulator, called PuTTY.

\textbf{\emph{Note:}} Remote TCP/IP communication protocol operation is
unaffected by the setting of network remote access for a Web browser
(discussed in the next section). Therefore, remote TCP/IP communication
can be enabled and used independently of the Web-browser remote access
setting (discussed below).

\hypertarget{network-remote-access-1}{%
\subsection{Network Remote
Access}

The following procedure provides steps to enable local network remote
access for GL controller to be accessed remotely via a Web browser for
control and operation of the chamber.
...
\item
  The GL controller system will reboot to apply the setting.
\item
  To confirm the setting (if it was applied successfully) following the
...
The following figure is the GL controller UI accessed on Google Chrome
via the IP address with:
...
\hypertarget{romfirmware-information}{%
...
\hypertarget{over-the-air-update}{%
...
\hypertarget{manual-update-via-hmi}{%
...
\hypertarget{manual-update-via-local-network}{%
...
\hypertarget{download-back-trace-data}{%
...
\hypertarget{create-back-trace-data}{%
...
\hypertarget{data-download-log-data-backup}{%
\section{Data Download (Log Data
Backup)}

Data log can be archived as data backup using Samba. This feature (and
this page) requires an administrator's privilege to configure the data
backup.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/2130403341-S6_10-data-download-001.PNG}
\caption{Data backup via Samba share}
\end{figure}\FloatBarrier
...
\item
  Log in as admin
\item
  Select the \textbf{Setup} menu (in the menu bar)
\item
  Select the \textbf{Data Download (Log Data Backup)} submenu
\end{enumerate}

A shared folder must be created on the target server (or computer) that
hosts a Samba share. Then complete the following steps:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Turn on Samba with the check mark in the box to enable the feature.
  \textbf{\emph{Note:}} If this box is not checked, all the required
  fields are grayed out (cf.~previous figure).

  \begin{figure}
  \centering
  
\includegraphics[width=0.95\textwidth]{figures/2919750157-S6_10-data-download-002.PNG}
  \caption{Completing the required fields for Samba data backup}
  \end{figure}\FloatBarrier
\item
  Set the backup frequency via the drop-down menu. Three options are
  available: (1) every 5 minutes (default), (2) Once per hour, and (3)
  Once per day.
\item
  Enter the correct path to the shared folder on your server.
\item
  The shared folder requires the complete credentials (username and
  password). Enter them in the appropriate fields, along with the
  correct domain as may be part of the shared folder.
\item
  Apply the \textbf{Save} button (red dot, bottom of menu bar) to save
  the settings.
\end{enumerate}

\hypertarget{accessory-gl-controller-power-management}{%
...
\hypertarget{restart-services-or-reboot-gl-controller}{%
...
\hypertarget{display-customization-home-and-submonitor-pages}{%
...
\hypertarget{widget-page-and-selection-options}{%
...
\hypertarget{general-widget-editing-procedure}{%
...
\hypertarget{general-widget-creating-procedure}{%
...
\hypertarget{configuration-menu}{%
...
\hypertarget{set-lan-network-interface-setup}{%
...
\hypertarget{lan-communication-test-hmi-or-web-display}{%
...
\hypertarget{lan-communication-test-putty}{%
...
\hypertarget{set-datetime-on-hmi}{%
...
\hypertarget{set-datetime-on-remote-display}{%
...
\hypertarget{ntp-server-setup-local-time-server}{%
...
\hypertarget{restore-debian-ntp-server}{%
...
\hypertarget{users-register-user-password}{%
...
\hypertarget{delete-or-remove-user-account}{%
...
\hypertarget{control-attainment-range}{%
...
\hypertarget{sensor-offset-calibration}{%
...
\hypertarget{set-chamber-details}{%
\section{Set Chamber Details}

The \textbf{Set Chamber Details} page is not available via the GL
controller software system; hence, it is grayed out.

\hypertarget{display-setup}{%
...
HMI display and the remote Web display. Only three options--startup
display tab, user interface theme and sub-tab position memorization--are
available for customization via the remote operation on the Web display
(see Section 7.9.2).

\hypertarget{display-setup-on-hmi}{%
\subsection{Display Setup on HMI}

The label associated with each button indicates its specific setting and
behavior. As depicted in the following figure, the highligted buttons
indicate the selected options. To customize the setting, press the
desired button.
...
\hypertarget{display-setup-on-web-display}{%
...
Several options are available for selecting a language. The following
procedures outline the steps to set and select a desired language for
the GL controller user interface. Language selection can be set under
any user account and via HMI or remote Web access.

\hypertarget{using-the-set-language-button}{%
\subsection{Using the Set Language
Button}

Select a language from within the Home page.

\textbf{\emph{Procedure 1:}}
...
\hypertarget{set-language-via-the-setup-menu}{%
\subsection{Set Language via the Setup
Menu}

Select a language via the menu bar.

\textbf{\emph{Procedure 2:}}
...
\hypertarget{set-language-via-the-hamburger-menu}{%
\subsection{Set Language via the Hamburger
Menu}

Select a language via the hamburger button.

\textbf{\emph{Procedure 3:}}
...
\hypertarget{set-account-recovery-email}{%
\subsection{Set Account Recovery
Email}
...
To access the Email setup page, be sure to log in as admin (see Section
2.6.3), then access the hamburger menu (in the status bar), select
Configuration, followed by Email. The figure below depicts the Email
page.

\textbf{\emph{Note:}} Section 2.11 also addresses this topic in details
...
\hypertarget{set-email-alert}{%
\subsection{Set Email Alert}
...
\hypertarget{how-to-recover-admin-account-and-password-1}{%
\subsection{How to Recover Admin Account and
Password}
...
is to reset the admin account to factory setting; and from there you can
change the admin password to something more secure. For this reason,
Section 12.1 (also see Section 2.11.1) should be done first and foremost
to limit security breach. Once an e-mail address has been set and stored
in the system, the recovery procedure will be sent to that e-mail
address.
...
\endhead
\begin{minipage}[t]{0.97\columnwidth}\raggedright
\textbf{Keep the beep enabled for instant alarm notification.} 2NEWLINE2
If the beep is turned off, alarm and warning are not notified with sound
and are indicated only with red blinking of the operation lamp and on
warning occurrence screen. 2NEWLINE2 \textbf{Set the beep volume in
accordance with the ambient environment.}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\endhead
\begin{minipage}[t]{0.97\columnwidth}\raggedright
\textbf{Do not attempt to change settings in the Service page.}
2NEWLINE2 Only ESPEC service representatives use this function to adjust
the setting parameters. Altering settings without authorization at your
own discretion can cause accidents. ESPEC is not responsible for damages
or injuries caused by user-adjusted parameters.\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\hypertarget{macro-editor-and-trigger-options}{%
...
\hypertarget{example-macro-script-with-alarm-alert}{%
\subsection{Example: Macro Script with Alarm
Alert}
...
\hypertarget{example-macro-script-with-program-state-and-alarm-alert}{%
...
\hypertarget{network-setup-remote-access}{%
...
\hypertarget{gl-controller-on-a-network}{%
...
\hypertarget{web-browser-compatibility}{%
\section{Web Browser
Compatibility}
...
\hypertarget{static-network-using-fallback-ip}{%
...
\hypertarget{static-network-connection-and-configuration}{%
...
\hypertarget{static-network-via-network-hub}{%
...
\hypertarget{custom-static-network}{%
...
drop-down menu displays a list of clickable links to other GL controller
or ESPEC Web Controller systems detected on the local network.
...
\hypertarget{network-monitor-page}{%
\section{Network Monitor Page}

The Network Monitor page can operate as a central server monitor where
all the GL controller and ESPEC Web Controller systems on the local
network can be displayed on a single monitor, as depicted in the
following figure. An operator can remotely monitor and manage all the GL
controller and ESPEC Web Controller systems from a centralized location.
In this discussion, we will refer to both the GL controller and other
ESPEC Web Controller systems as \textbf{\emph{EWC devices}} or
\textbf{\emph{EWC}}.
...
\hypertarget{managing-network-monitor}{%
...
\hypertarget{adding-a-new-monitor-tab}{%
...
\hypertarget{removing-an-ewc-device}{%
...
\hypertarget{adding-a-new-ewc-device}{%
...
\hypertarget{removing-a-monitor-tab}{%
...
\hypertarget{aboutlicense-terms}{%
\chapter{About/License Terms}

\textbf{About/License Terms} is the submenu accessible only via the
system menu (i.e., hamburger menu) in the status bar. It contains
important information about the GL controller software version, License
and Terms of Agreements for end-users and a link to ESPEC online support
page. The following procedure outlines the steps to access
\textbf{About/License Terms}.
...
\tightlist
\item
  Click the hamburger menu (see arrow)
\item
  Click the \textbf{About/License Terms} submenu (see arrow)
\item
  The first subpage of \textbf{About/License Terms} appears and the
  hamburger menu collapses (refer to Figure in Section 10.1).
\end{enumerate}

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/3073468097-gl-status-bar-sys-menu-000a.PNG}
\caption{Accessing About/License Terms in the status bar}
\end{figure}\FloatBarrier

This submenu provides three subpages, as depicted in the following
figure, with the hamburger menu shown as a pre-access action. Details of
these subpages are discussed in the following sections.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/3200454759-system-menu-firmware-002a.PNG}
\caption{Subpage 1: Software Information of GL Controller}
\end{figure}\FloatBarrier

\hypertarget{gl-software-information-espec-support-page}{%
\section{GL Software Information \& ESPEC Support
Page}

The first page (by default) displays general information of the GL
controller software, namely, its current firmware version, installed
version, package name and version number, date and time firmware was
created/installed, etc.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/2495806591-system-menu-001a.PNG}
\caption{GL Controller Software Information \& Support Page}
\end{figure}\FloatBarrier
...
1\strut
\end{minipage} & \begin{minipage}[t]{0.81\columnwidth}\raggedright
Displays the current firmware version of the GL controller
software\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.13\columnwidth}\raggedright
2\strut
\end{minipage} & \begin{minipage}[t]{0.81\columnwidth}\raggedright
Link to ESPEC online support page to access the GL controller user's
manual both online and PDF, and support information on other chamber
related requests\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.13\columnwidth}\raggedright
3\strut
\end{minipage} & \begin{minipage}[t]{0.81\columnwidth}\raggedright
Detailed information of the installed version of the GL controller\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}

The firmware version number (see arrow No.~1) can be used to check
against the current release of the GL controller software to ensure your
system is current and up to date. This firmware can also be used to
check against the original version shipped with the chamber to determine
whether a firmware update is needed.

If your GL controller is part of a network system with access to the
Internet and the GL controller is accessed via a Web browser from a PC,
the \emph{Web Controller Support Page} link (see arrow No.~2), when
clicked, points to the support page on ESPEC website for software
download or other support on chamber/software related questions, as
depicted in the following figure. Click the back-arrow button (in the
upper-left page) to return to the GL controller system.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/254095340-system-menu-support-page-001.PNG}
\caption{ESPEC customer support page}
\end{figure}\FloatBarrier

\textbf{\emph{Note:}} On the HMI, \emph{Web Controller Support Page}
link is blocked (inoperative), even when the GL controller system has
access to the Internet.

\hypertarget{end-user-agreement-and-copyright-notice}{%
\section{End-User Agreement and Copyright
Notice}

By default, subpage 2 opens and displays the overview of the software
license, its legal aspect, and its practical application and usage that
all end-users must acknowledge and abide to in order to use the product.

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/2924605868-system-menu-p2-copyright-end-user-001a.PNG}
\caption{End-User's Agreement \& Copy Right Notice}
\end{figure}\FloatBarrier

The main focus of this page is on end-user agreement and terms of
service. It lists different modules and functions used in the GL
controller software which are governed by various license terms and
copyrights. End-users are encouraged to view this page in its entirety,
including the respective licenses applied to each of them. This page
implies that, by using this software (i.e., GL controller software),
end-users have read, understood and accepted all the terms outlined
herein.

\hypertarget{embedded-gl-operation-manual}{%
\section{Embedded GL Operation
Manual}

The GL controller operation manual is available in two different
formats: online and PDF. Both formats are available via the support link
in Section 10.1 for download (PDF format) or online view via the wiki
page. The embedded manual (i.e., PDF format) is made available from
within the GL controller system without having to access the online
manual via the ESPEC support page.
...
\item
  Click the \textbf{About/License Terms} submenu (see arrow)
\item
  Click subpage 3 (see red arrow in the following figure)
\end{enumerate}

\begin{figure}
\centering

\includegraphics[width=0.95\textwidth]{figures/2864348590-system-menu-p3-embedded-manual-001b.PNG}
\caption{Embedded ESPEC GL Controller Software, User's Manual}
\end{figure}\FloatBarrier

By default, the first page (or title page) of the document is displayed
(see callout No.~1). To access a specific chapter or section, click the
table of contents (callout No.~2).

\textbf{\emph{Note:}} The embedded manual is not viewable on the HMI;
the GL controller system does not have a PDF viewer to display the
manual.

\hypertarget{appendix}{%
\section{APPENDIX}

\hypertarget{a.1-program-creation-sheet}{%
...
\hypertarget{a.2-program-creation-sheet}{%
...
\hypertarget{b.1-specification-of-hmi-touch-screen-display}{%
...
\hypertarget{c.1-specification-of-aaeon-up2board}{%
