...
\subtitle{Service Tech. Training on Software \& Configuration}
...
ESPEC Web Controller V3 (EWC V3) is an embedded computer. Its hardware is based on an x86 architecture, called Up2Board (pictured below). It is powered by Debian GNU/Linux distribution as its operating system.  
\begin{figure} [hbt!] 
\centering
...
\begin{frame}{1.1: ESPEC Web Controller V3: Features}{} 
\begin{itemize}
...
\begin{frame}{1.2: Updates \& Security Features}{} 
\begin{itemize}
% Here is another example of security issues.
   \item {\bf{Security:}} With Debian GNU/Linux periodically receiving security updates, EWC V3 is kept extremely secure. 
EWC V3 is configured with read-only access to prevent intruders from exploiting its system. 

   \item {\bf{Updates \& Stability:}} EWC V3 firmware update is managed by a cloud service provider (called MenderIO)  which employs a dual-root (stepladder) technique to provide a smooth update. One root system is always in a stable state to ensure an uninterrupted operation, should the firmware update fail.

   \item {\bf{Access Privilege \& User Policy:}} EWC V3 is shipped with one user account (called {\bf{admin}}) with complete powers to manage the system. This {\bf{admin}} account can enforce a user policy on EWC V3 and administer additional accounts with various privileges. Only authorized users can log into and control EWC V3.
...
EWC V3 supports a touchscreen or a standard display.
\begin{itemize}
   \item {\bf{Touchscreen:}} 
    With a touchscreen or a standard display attached to its display port, 
    EWC V3 automatically displays its UI for chamber control and operation. 
    \begin{figure} [hbt!] 
      \centering
...
Advantages of display options:
\begin{itemize}
   \item {\bf{Touchscreen:}} Default configuration
         \begin{enumerate}
             \item T-series chambers are shipped with the MIMO touchscreen to provide 
                   the UI of EWC V3 as its dedicated HMI (see Figure in previous slide). 
             \item A custom chamber with F4T is equipped with the MIMO 
                    touchscreen to provide the UI of EWC V3 as its dedicated HMI.
         \end{enumerate} 
   \item {\bf{Standard Monitor:}} Optional or Practical
          \begin{itemize}
               \item With standard display, mouse and keyboard attached, EWC V3 becomes 
                     as a stand-alone computer to operate and control the chamber. 
          \end{itemize} 
   \item {\bf{Advantage of Touchscreen or Standard Display:}} 
          \begin{itemize}
               \item Operating as a stand-alone device, EWC V3 does not need to be part of a network to control the chamber.   
...
EWC V3 uses chamber S/N as its registration number to receive firmware update through our cloud service provider, MenderIO.
    \begin{figure} [hbt!] 
      \centering
...
\begin{frame}{4.1: User Interface: Web Controller Home Page}{}
...
% SECTION 5
\section{5. Configuration at the Factory} 

\begin{frame}{5.0: Web Controller Install and Setup}
Production has a check list to complete the Web Controller installation and configuration
prior to shipping the product.

    \begin{figure} [hbt!] 
      \centering
      \includegraphics[scale=.30]{figures/new-cfg-passed-001.PNG}  
      \caption{ESPEC Web Controller setup checklist}
    \end{figure}

Installation and configuration are complete when all four items 
...
\begin{frame}{5.1: Factory Configuration: Incomplete}
Examples of incomplete factory configuration of Web Controller. 

    \begin{figure} [hbt!] 
      \centering
      \includegraphics[scale=.18]{figures/1919602414-wizard-cfg-002.PNG}  \\
      \caption{All failed}
    \end{figure}
Customer (or Service Tech) will see this page when they first start the Web Controller. 
\end{frame}

\begin{frame}{5.2: Factory Configuration: Incomplete}{}
Examples of incomplete factory configuration of Web Controller. 

    \begin{figure} [hbt!] 
      \centering
      \includegraphics[scale=.32]{figures/verification-failed-001a.PNG}  \\
...
\begin{frame}{5.3: Factory Configuration: Incomplete}{}
Examples of incomplete factory configuration of Web Controller. 
...
\section{6. Chamber Support \& Interface, Network Configuration} 
\begin{frame}{6.0: Communication Interface \& Configuration}{}
...
\begin{frame}{6.1: DHCP \& Static Nework Configuration}{}
\begin{itemize} 
...
\begin{frame}{6.3: Chamber Interface Configuration: F4T}{} 
...
\begin{frame}{6.4: TCP/IP Configuration for F4T}{}

Default internal network between EWC V3 and F4T is LAN protocol via Network Interface options (see Figure below).
...
Default IP configurations are as follows: 
  ...
  \begin{itemize}
     ...
     \item {\bf{Network}}: Internal Network protocol: 10.30.200.240/28. 
     \item {\bf{Server IP}}: EWC V3 IP address: 10.30.200.241.
     \item {\bf{Server IP}}: F4T IP address: 10.30.200.242.
  \end{itemize} 
...
\begin{minipage}[c]{0.450\textwidth} 
        \includegraphics[width=\textwidth]{figures/f4t-network-cfg-002.PNG}  
%        \caption{Example of F4T internal network setting}
        \captionof{figure}{F4T IP config}
%      \end{figure}
\end{minipage}

\vspace{0.125in} 
Step-by-step procedure for configuring F4T internal network is provided in the {\bf{Troubleshooting Guide}}.

\end{frame} 


\begin{frame}{6.5: TCP/IP Options for F4T}{}
...
\begin{frame}{6.6: TCP/IP Configuration for T-Series}{}
...
EWC V3 uses its predefined Ethernet port to connect to the customer's main network, depicted in the diagram (right).  
\end{minipage}
\hfill
...
%        \includegraphics[scale=.20]{figures/f4t-network-cfg-001.PNG}  
        \includegraphics[width=\textwidth]{figures/up2board-io-002.PNG} 
%        \caption{Example of F4T internal network setting}
        \captionof{figure}{Ethernet ports}
%      \end{figure}
\end{minipage}
\end{frame}


\begin{frame}{6.7: TCP/IP Options for T-series}{}
...
\begin{frame}{6.8: Chamber Interface Configuration: T-series}{} 
Communication between Web Controller and Chamber is possible only when chamber interface configuration is done correctly, as shown in the following screenshots. 
...
\begin{frame}{6.9: Up2Board Connecting to F4T or T-Series}{} 
EWC V3 and F4T communicate through TCP/IP interface as shown in the followign diagram. 
Step-by-step procedure for configuring F4T internal network is provided in the {\bf{Troubleshooting Guide}}. 
...
For T-series chambers, the Enternet cable from the chamber will go into the Up2Board's Ethernet port (shown above in of F4T). The cable running from Up2board's main network to will out to customer's main network.  
  
\end{frame}


\begin{frame}{6.10: Serial Interface Configuration}{}
...
\begin{frame}{6.11: P300 with RS-485}{} 
...
\begin{frame}{6.12: Connecting to P300/SCP220/ES102/F4}{} 
RS-232 is the standard communication interface for SCP220/ES102 and F4. P300 uses its 3-pin RS-232C for communication. Correct pin configuration on the Up2Board is required for RS-232: 

    \begin{figure} [hbt!] 
      \centering
      \includegraphics[scale=.37]{figures/scp220-jumper-setting-0a.PNG}  \\
      \caption{RS-232 jumper setting on Up2Board for ES102, SCP220, F4 and P300 (3-pin) COMM}
    \end{figure}
\end{frame}

\begin{frame}{6.13: Chamber Interface Configuration: P300}{} 
Communication between Web Controller and Chamber is possible only when chamber interface configuration is done correctly, as shown in the following screenshots. 

    \begin{figure} [hbt!] 
      \centering
      \includegraphics[scale=.220]{figures/p300-cfg-001.PNG}  \\
      \caption{Setup Wizard chamber interface configuration for P300}
    \end{figure}

\end{frame}

\begin{frame}{6.14: Chamber Interface Configuration: SCP220}{} 
Communication between Web Controller and Chamber is possible only when chamber interface configuration is done correctly, as shown in the following screenshots. 
...
\section{7. Reconfigure Chamber Interface Communication}

\begin{frame}{7.0: Communication Error: No response}{} 
...
Possible causes: 
\begin{itemize}
   \item Serial cable uplugged.
   \item Ethernet cable (F4T/T-series) unplugged.
   \item Chamber is off.  

\end{itemize}
\end{frame}

\begin{frame}{7.1: Communication Error: No response}{} 
...
\begin{frame}{7.2: Chamber Interface Config: Settings Menu}{} 
...
\begin{frame}{7.3: Chamber Interface Config: Settings Menu}{} 
...
\begin{frame}{7.4: Chamber Interface Config: Settings Menu}{} 
...
\section{8. Universal Web Controller Kit} 
...
\begin{frame}{8.1: U-Web Kit Support}{} 
...
\section{9. EWC V3 Features \& Operation}

\begin{frame}{9.0: EWC V3: Its Menu Bar}{}
\begin{minipage}[c]{0.50\textwidth}
...
\begin{frame}{9.1: EWC V3 Main Menu \& Features}{} 
...
\section{10. Hands-On: Demo} 

\begin{frame}{9.0: Using EWC V3, Hands-On}{}
\begin{itemize}
