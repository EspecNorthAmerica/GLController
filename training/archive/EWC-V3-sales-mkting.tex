...
\subtitle{Introducing Web Controller V3: Marketing}
...
and capabilities. ESPEC Web Controller V3 (EWC V3) is an embedded computer. 
...
\begin{itemize}
   \item EWC V3 uses a standard Web browser to provide (i.e., host) its user interface (UI) to control the chamber; hence, the name {\bf{Web Controller}}. 

   \item EWC V3 can operate as part of a network or a standalone machine. On a network, it hosts its UI via a Web browser accessable through its IP address or hostname. Thanks to a Web browser ability to operate from any computer on the network, chamber operations can be performed via EWC V3 remotely by authorized users on the network. 

   \item No device outside the network can access EWC V3, unless the network is configured to host port forwarding to allow EWC V3 be visible on the Internet. 
...
\begin{itemize}
   \item {\bf{Security:}} GNU/Linux is a secure system which constantly receives security updates. 
It is furthermore configured with Read-Only access to prevent intruders from exploiting the system; 
hence, EWC V3 is extremely secure. Only authorized users from a PC on the same network can access and control EWC V3. 

   \item {\bf{Stability:}} EWC V3 uses a dual-root (stepping ladder) configuration to provide a smooth update. One root system is
always in a stable state to provide uninterrupted operation. 

   \item {\bf{Access Privilege \& User Policy:}} EWC V3 is shipped with an 
administrator account. This administrator can set user policy on EWC V3 by creating 
additional accounts with various privileges. Only authorized users can access and control EWC V3.

\end{itemize}
\end{frame} 


\begin{frame}{1.3: Touchscreen and Standard Display}{}
EWC V3 now supports a touchscreen operation compared to its predecessor, V2.5.
\begin{itemize}
   \item {\bf{Touchscreen:}} 
...
\end{frame}    
%
\begin{frame}{1.5: Cloud Updating}{} 
\begin{itemize}
...
\begin{frame}{1.7: Display Options}{}
