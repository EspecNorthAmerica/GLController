...
\subtitle{T-Series HALT/HASS Chambers}
...
ESPEC Web Controller V3 (EWC V3) is an embedded computer. 
Its hardware is based on an x86 architecture, called Up2Board (pictured below). 
It is powered by Debian GNU/Linux distribution as its operating system.  
\begin{figure} [hbt!] 
\centering
\includegraphics[scale=0.25]{figures/tseries-up2-001.PNG}
\caption{EWC Up2Board with Ethernet and I/O ports}
\end{figure}
\end{frame}
...
\begin{itemize}
   ...
   \item EWC V3 can operate as a standalone machine using a touchscreen or a 
         standard display. It uses a subset of a Web browser, called Electron, 
         to host its UI on the display.
...
\begin{itemize}
   \item {\bf{Touchscreen:}} 
    ...
    \begin{figure} [hbt!] 
      ...
      \includegraphics[scale=0.30]{figures/tseries-toucscreen-002-fix2.PNG}
...
\begin{itemize}
   ...
   \item {\bf{Advantage of Touchscreen or Standard Display:}} 
          \begin{itemize}
               \item Operating as a stand-alone device, EWC V3 does not need to be part 
                     of a network to control the chamber.   
