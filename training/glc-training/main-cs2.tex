...
GL Chamber will eventually replace P300-based (and/or SCP-based) chambers. The Platinous Series based on GL controller has been released. Work on Global chamber is underway. 
\begin{figure} [hbt!] 
\centering
...
\vspace{0.1in}
The software part is written in CODESYS; its details do not concern us! This software part works with our EWC with some modifications to allow GL run on the EWC hardware (Up2Board) in the figure below. 
\begin{figure} [hbt!] 
      \centering
...
\vspace{1em} GL controller {\it{is}} the CODESYS that resides on the EWC platform. EWC and CODESYS are the GL controller. The GL controller uses Up2Board (EWC hardware) to link to its Input/Output board, depicted in figure on the right. 
\end{minipage} 
\hfill
...
\end{itemize}
The next slide depicts enable or disable options for remote communication and Web access. 
\end{frame}
...
\begin{itemize} 
    ...
    \item The Library will be integrated with the current Chamber Connect Library for P300, SCP220, ES102 and Watlow F4 and F4T. \\

    \vspace{2em} 

    \item Communication protocol and interface for GL include TCP and Serial. GP-IB communication protocol is currently not supported. 
\end{itemize} 
\end{frame}
...
\begin{itemize}
   ...
   A demo chamber to familiarize with the GL software UI and operation is available at this IP address: \\
   \vspace{1em}
   {\url{http://10.30.200.247/}} \\
   {\it{username:}} cservice \\ {\it{password:}} cservice \\
   \vspace{1em}  
   It must be accessed locally (at MI facility) or via VPN. Note: Availability of the above chamber is subject to change. Notify me if you cannot access it. \\
   \vspace{1em}
   We will soon have a GL demo for potential customers on a public network (public IP address on the Internet). 
